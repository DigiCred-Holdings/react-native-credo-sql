...
export function didcommV1Pack(
  ...
  try {
    ...
    senderExchangeKey = senderKey
      ? senderKey.convertkey({ algorithm: KeyAlgs.X25519 })
      : undefined;
      ...
      try {
        ...
        if (senderKey && senderExchangeKey) {
          const encryptedSender = CryptoBox.seal({
            ...
            message: TypedArrayEncoder.fromString(
              TypedArrayEncoder.toBase58(senderKey.publicBytes)
            ),
          });
...
export function didcommV1Unpack(
  ...
  const recipient = protectedJson.recipients.find(
    ...
    (r: any) =>
      r.header.kid === TypedArrayEncoder.toBase58(recipientKey.publicBytes)
  );
  ...
  const sender = recipient?.header.sender
    ? TypedArrayEncoder.fromBase64(recipient.header.sender)
    : undefined;
  const iv = recipient?.header.iv
    ? TypedArrayEncoder.fromBase64(recipient.header.iv)
    : undefined;
